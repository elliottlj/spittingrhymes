## About this game

Type words that rhyme with the target word before time runs out. **True rhymes** (+10) sound identical from the stressed vowel onward. **Slant rhymes** (+2) share only the vowel sound or only the ending. Everything is checked against the [CMU Pronouncing Dictionary](http://www.speech.cs.cmu.edu/cgi-bin/cmudict) (~126,000 words), so some accepted rhymes may be obscure.

## FAQ

### What is this game for?

Helping you to learn which words rhyme. This could be because you want to use rhyming words in an improv performance or practice English. It is inspired by the game [RhymeRush](https://rhymerush.com/), but crucially it also tells you the rhymes that you missed!

### Why wasn't my word accepted?

Either it doesn't rhyme based on the dictionay pronunciation, or it isn't in the dictionary. The dictionary is the [CMU Pronouncing Dictionary](http://www.speech.cs.cmu.edu/cgi-bin/cmudict) which uses Americation pronunciations and has a few words missing.

### Can I share a round with someone?

Yes - once a round ends, press "Copy link" and share it. Whoever you send it to plays the exact same round.

### Is my stats history saved anywhere online?

No. It's stored only in your browser's local storage — nothing is uploaded anywhere.

### Can I clear my history?

Yes, the trash icon in the Stats panel clears it. This can't be undone.

### What's a stressed vowel?

Coming soon!
